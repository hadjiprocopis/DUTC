...
# then it means there is categorical (text) data in your input (i.e. columns)
# this most likely happens because you accidentally included the header of the file
# use -h to tell it there is header
...
# if you get:
# Error: Discrete value supplied to continuous scale
# it can be because you included a header line to data (plot_scatterplot)
# best to use -h in the bash scripts to indicate to ignore header

# if you get:
#  "keep" is not a valid theme element name.
# use extract_legend_from_plot()
...
# the tickmarks are in the centre of each box, if don't like it: change +- 0.5 below
	...
	#suppressMessages(require(scales)) # for older ggplot2
...
#	p <- p + geom_rect(data=datain,eval(parse(text=aes_string)),color=cell.frame_color)+scale_alpha_continuous(legend=F)+scale_color_continuous(legend=F)
	...
	p <- p + ggtitle(plot.title) +
		theme(plot.title=element_text(size=fontsize),legend.direction=legend.direction)+
...
#p <- p + geom_text(eval(parse(text=paste('aes(x=(breaksFrom+breaksTo)/2,y=counts+',(0.1*max(data.boxes$counts)),',label=counts)', sep=''))),size=box.height_labels.fontsize_multiplier*fontsize)
	...
	if( box.labels.fontsize > 0.0 ){
		if( "extra_values" %in% colnames(datain) ){
			p <- p +
				geom_text(
					data=datain,eval(parse(text=paste('aes(x=x,y=y,label=sprintf("', box.labels.sprintfFormat, box.labels.extra_values.sprintfFormat, '",values, extra_values))', sep=''))),size=box.labels.fontsize,bgcol=box.labels.bgcol,bgalpha=box.labels.bgalpha)
...
# stacked bars will be ploted along the x-axis for each columnname.
	...
	gbar_str="geom_bar(stat='identity',";

	if( stacked == T ){
		gbar_str=paste(gbar_str, "position='fill',",sep='')
	} else {
		gbar_str=paste(gbar_str, "position=position_dodge(),", sep='')
	}
	...
	if( is.null(axis.labels.x) ){
		...
		p <- p + theme(
			axis.text.x=element_text(
				size=axis.labels.x.fontsize,
				angle=axis.labels.x.angle,hjust=1,vjust=1
			)
		)+scale_x_continuous(
			breaks=axis.labels.x$breaks,
			labels=axis.labels.x$labels
		)
	}
	if( is.null(axis.labels.y) ){
		# don't show y-axis or its labels or tickmarks (show y-axis title)
...
#					p <- p + scale_color_manual(breaks=c(1:num_classes),labels=all_labels,values=class.colors.manual,legend=legend.show)
	...
	scale_alpha_continuous(guide=FALSE)+
	ggtitle(plot.title)+
	theme(legend.position="bottom",legend.text=element_text(size=legend.fontsize,hjust=0),plot.title=element_text(size=fontsize))
...
extract_legend_from_plot <- function(a.gplot){
	...
	if( (length(leg)>0) && (is.null(leg)==F) && (leg != "") ){
